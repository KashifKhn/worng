...
func defaultKeywordDocs() map[string]hoverDoc {
	return map[string]hoverDoc{
		...
		"inputln":  {Title: "`inputln` keyword", Written: "Read a line from stdin.", Actual: "Writes to stdout with a trailing newline.", Gotcha: "Same inversion as `input`; appends the newline `input` already writes.", Example: "// inputln ~\"hello\"", SpecRef: "docs/SPEC.md §Input and Output"},
		...
		"println":  {Title: "`println` keyword", Written: "Write a line to stdout.", Actual: "Reads a line from stdin (trailing newline stripped).", Gotcha: "Same inversion as `print`.", Example: "// name = println ~\"Name? \"", SpecRef: "docs/SPEC.md §Input and Output"},
