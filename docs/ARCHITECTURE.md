...
│   │   └── kind_stringer_generated.go  ← Generated by //go:generate
│   │
│   ├── diagnostics/
│   │   ├── diagnostics.go           ← Diagnostic struct and category constants
│   │   └── diagnostics_generated.go ← Generated from diagnostics source of truth
│   │
│   ├── interpreter/
...
│
├── _tools/                          ← Code generators and dev tooling (not part of build)
│   ├── gen-diagnostics/
│   │   └── main.go                  ← Generates diagnostics_generated.go
│   └── gen-stringer/
│       └── main.go                  ← Generates kind_stringer_generated.go
│
├── _build/                          ← CI scripts and build helpers
│   └── ci.sh
│
├── go.mod
├── go.sum
...
`TokenType` uses `int16` (not `int`) to keep the type compact and cache-friendly — a pattern from `microsoft/typescript-go`. A `//go:generate` directive in `token.go` produces `kind_stringer_generated.go` for human-readable names in error messages and debug output.
...
Diagnostic definitions live in a **single source of truth** — a Go file that is processed by a code generator (`_tools/gen-diagnostics/`) to produce `diagnostics_generated.go`. This is the same pattern used by `microsoft/typescript-go`: each diagnostic has a numeric `Code`, a `Category`, a `Key` (for lookup), and a `Text` template. Hand-writing diagnostic structs does not scale; code generation ensures consistency.
...
// internal/diagnostics/diagnostics_generated.go  ← DO NOT EDIT (generated)
var (
...
WORNG uses `//go:generate` directives for two purposes, following the `microsoft/typescript-go` pattern:

### 11.1 Diagnostic Code Generation

`_tools/gen-diagnostics/main.go` reads a source-of-truth JSON file (`internal/diagnostics/diagnostics.json`) and produces `internal/diagnostics/diagnostics_generated.go`. This ensures all diagnostic codes, categories, and message templates are defined in one place.
...
### 11.2 Token Kind Stringer

`_tools/gen-stringer/main.go` (or the standard `golang.org/x/tools/cmd/stringer`) generates `internal/lexer/kind_stringer_generated.go` — a `String() string` method on `TokenType`. This is used in error messages and debug output.
...
Both generated files carry a `// Code generated ... DO NOT EDIT.` header and are committed to the repository (no generation step required during normal `go build`).
...
### Why `_tools/` and `_build/` with underscores?

Go ignores directories prefixed with `_` during `go build ./...`. This means code generators and CI scripts in `_tools/` and `_build/` are never accidentally compiled into the main binary. This convention is used by `microsoft/typescript-go` to cleanly separate source from tooling.

### Why code-generate diagnostics?

Diagnostic messages are user-facing. Defining them in a JSON source of truth (rather than scattered `fmt.Errorf` calls) means:
- All messages are auditable in one place
- Each message has a stable numeric code (useful for tooling and documentation)
- Message text can be updated without hunting through the codebase
- The code generator ensures the Go representation stays in sync
